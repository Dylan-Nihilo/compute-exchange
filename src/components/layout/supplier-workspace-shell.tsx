...
const navItems: readonly {label: string; icon: string; href: string}[] = [
  ...
  {label: "数据看板", icon: `${supplierAssets}/chart-no-axes-combined.svg`, href: "/console/supplier/analytics"},
  {label: "消息中心", icon: `${buyerAssets}/message-square.svg`, href: "/console/supplier/messages"},
];
  ...
  return (
    <div className="omnis-workbench-controls relative min-h-screen overflow-x-clip bg-linear-to-r from-[#f3fbfe] via-[#f9fdff] to-[#fdfeff] text-[#102b3b]">
      <Image
        ...
        height={620}
        priority
        src={`${buyerAssets}/atmosphere-ice.svg`}
        width={620}
      />
      <Image
        alt=""
        ...
        height={360}
        src={`${buyerAssets}/atmosphere-cyan.svg`}
        width={520}
      />
        ...
        <Navbar.Header className="h-[72px] px-5 sm:px-10">
          ...
          <Navbar.Content className="gap-2">
            <Button
              ...
              onPress={() => router.push("/console/supplier/messages")}
              variant="ghost"
            >
              <Image alt="" aria-hidden="true" height={16} src={`${buyerAssets}/notification.svg`} width={16} />
              {unreadCount > 0 ? (
            ...
            <Dropdown>
              ...
              >
                ...
                <span className="hidden min-w-0 sm:block">
                  ...
                  <span className="block text-[10px] text-[#78909c]">供给方</span>
                </span>
                <Image alt="" aria-hidden="true" height={16} src={`${buyerAssets}/chevron-down.svg`} width={16} />
              </Button>
        ...
        <Sidebar className="hidden min-h-[calc(100vh-72px)] w-[212px] shrink-0 border-0 bg-transparent px-4 py-6 shadow-none lg:flex">
          <Sidebar.Content className="gap-1.5 overflow-visible px-0">
            <Sidebar.Group>
              ...
              <Sidebar.Menu aria-label="供给方工作台导航" showGuideLines={false}>
                {navItems.map((item) => {
                  const isCurrent =
                    pathname === item.href ||
                    (item.href !== "/console/supplier" && pathname.startsWith(`${item.href}/`));
                  ...
                  return (
                    ...
                    >
                      <Sidebar.MenuItemContent>
                        <Sidebar.MenuIcon className="text-[#5e7786]">
