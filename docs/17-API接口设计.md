...
> 2026-08-20 契约口径：手机号短信验证码注册、登录已进入联调；邮箱验证码和微信扫码仍待真实渠道。账号密码与企业微信后置。

| 方法 | 路径 | 说明 | 鉴权 |
|------|------|------|:--:|
| POST | `/auth/sms/code` | Cap 校验后发送注册/登录短信验证码 | ❌ |
| POST | `/auth/register` | 手机号验证码注册并返回会话，新账户默认 buyer | ❌ |
| POST | `/auth/sms/login` | 手机号验证码登录并返回会话 | ❌ |
| POST | `/auth/refresh` | 刷新 token | ❌（用 refresh_token） |
| POST | `/auth/logout` | 幂等登出 → access/refresh token 同时撤销 | ❌（token 自证） |
...
// Response
{ "code": 0, "data": { "expires_in": 300, "resend_after": 60 } }
```

`expires_in` 表示验证码有效期，`resend_after` 表示重新获取冷却时间。

**POST /auth/register**
```json
// Request
{ "phone": "13800138000", "sms_code": "123456", "agree_tos": true }
// Response
{ "code": 0, "data": {
...
**GET /products**
```
Query: ?q=H100&product_type=card_rental&gpu_model=H100&region=北京&delivery_mode=container&pricing_mode=hourly&available_hours=夜间&price_min=2000&price_max=5000&card_count_min=8&sort=price_asc&page=1&page_size=20
```

`price_min` / `price_max` 使用分；`sort` 支持 `created_at_desc`、`price_asc`、`price_desc`、`stock_desc`；`page_size` 最大 100。
