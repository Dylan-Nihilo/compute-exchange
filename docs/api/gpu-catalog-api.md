...
运营与管理员可进入 `/admin/gpu-catalog`，按型号、厂商、产地和级别查询，并使用现有 POST/PUT 新增或整体更新。未核实规格留空时提交 `null`，不补为 0；来源字段只在运营页面显示。

停用或修改标准型号名称前展示影响确认。停用从公开选择器隐藏，但不删除条目或商品/订单的历史型号快照；名称变更也不改写已发布商品。保存失败保留表单，重复型号等服务端错误如实显示；没有新增 DELETE 或自动回填接口。

市场型号筛选与发布选择器共用公共型号接口，选项原样使用 `model_name`；每次打开筛选时刷新。当前查询使用已停用或历史名称时仍保留该值，避免把已有查询替换为其他型号。
