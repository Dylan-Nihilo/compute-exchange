...
登录后可从任意角色工作台的账户菜单进入 `/console/consents`。页面使用现有 `GET /api/auth/consents`，展示文档、实际版本、同意场景、关联编号和时间；接口为最近 100 条的数组，不伪造服务端分页或历史同意。

已收录正文按记录版本在新标签页打开。当前只有首版；未收录的文档或版本会明确显示正文不可用，不能链接到当前正文冒充历史版本。错误与空记录分别展示，未登录仍由工作台访问边界和后端鉴权保护。
